 ...
 */
export async function handleMyWalletButton(ctx: Context): Promise<void> {
  const telegramId = ctx.from?.id.toString();
  if (!telegramId) return;
  ...
  try {
    ...
    const walletText = `Wallet Overview
...
PIN Set: ${user.pin_hash ? 'Yes' : 'No'}`;
    ...
    await ctx.editMessageText(
      walletText,
      {
        parse_mode: 'Markdown',
        ...Markup.inlineKeyboard([
          [Markup.button.callback('Export Private Key', CALLBACK.EXPORT_KEY)],
          [Markup.button.callback('Add New Wallet', CALLBACK.ADD_WALLET)],
          [Markup.button.callback('Remove Wallet', CALLBACK.REMOVE_WALLET)],
          [Markup.button.callback('Back', CALLBACK.BACK_MAIN)],
        ]),
      }
    );
  } catch (error) {
    console.error('[Menu] My Wallet button error:', error);
    await ctx.answerCbQuery('Something went wrong. Try again.');
  }
}
...
This feature creates a new trading wallet.

Current system uses one wallet per user.
Contact support for multi-wallet access.`,
      Markup.inlineKeyboard([
        [Markup.button.callback('Back', CALLBACK.BACK_MAIN)],
      ])
    );
  } catch (error) {
    console.error('[Menu] Add Wallet button error:', error);
    await ctx.answerCbQuery('Something went wrong. Try again.');
  }
}
